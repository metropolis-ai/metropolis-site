#!/usr/bin/env node
/*
 * sync-doc.cjs — copy a vault markdown node into the marketing site as a gated doc.
 *
 * Usage:  node sync-doc.cjs <path-to-vault-node.md> [--slug SLUG]
 *
 ...
 * is git-ignored plaintext; `build.cjs` encrypts it into the gated hub, where it
...
if (!src || !fs.existsSync(src)) {
  console.error('Usage: node sync-doc.cjs <path-to-vault-node.md> [--slug SLUG]');
  process.exit(1);
}
...
console.log(`Next: ensure a TREE node in build.cjs uses  src: '${outName}'  then run  node build.cjs`);
